...
local levelNameStatusStringCategory = "|T%s:16:16:0:0:64:64:4:60:4:60|t |cff%02x%02x%02x%d|r %s%s "
local nameRankString = "%s |cff999999-|cffffffff %s"
local nameRankStringCategory = "%s |cff999999-|cffffffff %s - %s"
...
local function OnClick(_, btn)
	if btn == "RightButton" and IsInGuild() then
		...
		EasyMenu(menuList, GuildDatatTextRightClickMenu, "cursor", 0, 0, "MENU", 2)
	else
		GuildFrame_Toggle()
	end
end
...
local function OnEnter(self, _, noUpdate)
	...
	for i = 1, #guildTable do
		...
		if IsShiftKeyDown() then
			if info[12] then
				DT.tooltip:AddDoubleLine(format(nameRankStringCategory, info[1], info[2], info[12]), info[4], classc.r, classc.g, classc.b, zonec.r, zonec.g, zonec.b)
			else
			...
			if info[11] then
				DT.tooltip:AddDoubleLine(format(levelNameStatusStringCategory, info[11], levelc.r*255, levelc.g*255, levelc.b*255, info[3], info[1], groupedTable[grouped], info[8]), info[4], classc.r,classc.g,classc.b, zonec.r,zonec.g,zonec.b)
			else
